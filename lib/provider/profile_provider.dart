...
class ProfileProvider with ChangeNotifier {
  ...
  Future<ResponseModel> updateUserInfo(UserInfoModel updateUserModel, String password,String token) async {
    ...
    notifyListeners();
    ResponseModel _responseModel;
    http.StreamedResponse response = await profileRepo!.updateProfile(updateUserModel, password, token);
    _isLoading = false;
  ...
  Future<http.StreamedResponse> updateWorkerBio(String token, String bio) async {

    http.MultipartRequest request = http.MultipartRequest('POST', Uri.parse('${AppConstants.BASE_URL}${AppConstants.UPDATE_WORKER_BIO_URI}'));
    request.headers.addAll(<String,String>{'Authorization': 'Bearer ${token}'});
    if(_profileImageFile != null) {
      print('step 2');
      print('----------------${_profileImageFile!.readAsBytes().asStream()}/${_profileImageFile!.lengthSync()}/${_profileImageFile!.path.split('/').last}');
