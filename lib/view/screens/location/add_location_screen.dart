...
class _AddLocationScreenState extends State<AddLocationScreen> {
  ...
  void initState() {
    ...
    if(Provider.of<LocationProvider>(context, listen: false).searchedLocation != null){
      initialPosition = LatLng(
          Provider.of<LocationProvider>(context, listen: false).latitude ?? 0.0,
          Provider.of<LocationProvider>(context, listen: false).longitude ?? 0.0);
    }else {
      initialPosition = LatLng(
          Provider.of<LocationProvider>(context, listen: false).position!.latitude ?? 0.0,
          Provider.of<LocationProvider>(context, listen: false).position!.longitude ?? 0.0);
    }
          ...
          child: Container(
            ...
            child: Consumer<LocationProvider>(
              builder: (context, locationProvider, child) {
                ...
                String? lng;
                if(Provider.of<LocationProvider>(context, listen: false).searchedLocation != null){
                  lat = Provider.of<LocationProvider>(context, listen: false).latitude.toString();
                  lng = Provider.of<LocationProvider>(context, listen: false).longitude.toString();

                }else {
                  lat = Provider.of<LocationProvider>(context, listen: false).position!.latitude.toString();
                  lng = Provider.of<LocationProvider>(context, listen: false).position!.longitude.toString();
                }
                  ...
                  InkWell(
                    onTap: () {
                      Navigator.pushReplacement(context, MaterialPageRoute(builder: (BuildContext context)=>
                          AutocompleteSearch(newArea: true)));
                    },
                    child: Container(
                      height: 45,
                                      ...
                                      if(_areaController!.text.isEmpty){
                                        ...
                                        locationProvider.storeNewArea(
                                            ...
                                            locationProvider.position!.longitude.toString()).then((value) {
                                              if(value.isSuccess){
                                                showCustomSnackBar('Area added successfully', context, isError: false);
