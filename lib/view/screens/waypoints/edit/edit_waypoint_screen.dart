          ...
          return Column(
            children: [
                      ...
                      child: SizedBox(
                        ...
                        child: Column(
                          ...
                          children: [
                                                ...
                                                MaterialButton(
                                                  ...
                                                  onPressed: () {
                                                    ...
                                                    Navigator.push(context,
                                                      MaterialPageRoute(builder: (BuildContextcontext) => SelectAreaScreen(),),
                                                    ).then((value) {locationProvider.markers.clear();
                                                      Navigator.pop(context);});
                                                  },
                                                  child: Text("Yes"), height: 40, color: Colors.white, elevation: 0,
                                                ),
                                ...
                                ? InkWell(
                                    onTap: () {
                                      ...
                                      Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                              builder: (BuildContext context) => locationProvider.markers.isEmpty
                                                      ? AddPinPointScreen(area: locationProvider.searchedArea)
                                                      : AddAndPressPinPointScreen(waypoint: widget.waypoint,)));
                                    },
                                    child: Container(
                                      height: 50,
                                      decoration: BoxDecoration(
                                          ...
                                          border: Border.all(color: Theme.of(context).primaryColor.withOpacity(0.4), width: 1)),
                                      child: Padding(
                                        padding: const EdgeInsets.all(8.0),
                                        child: Row(
                                          children: [
                                            const SizedBox(width: 10),
                                            Text(locationProvider.markers.length > 0
                                                  ? '${locationProvider.markers.length} Pins Added'
                                                  : 'Add Pins +',
                                              style: TextStyle(color: locationProvider.markers.isNotEmpty ? Colors.black87 : Colors.black45,
                                                  fontWeight: FontWeight.normal,fontSize: 14, fontFamily: 'Roboto'),),
                                            const Spacer(),
                                            const Icon(Icons.location_pin, color: Colors.black54, size: 25,),
                                          ],
                                        ),
                                      ),
                                    ),
                            ...
                            Text('Day', style: TextStyle(color: Theme.of(context).textTheme.headline2!.color,fontSize: 15),),
                            const SizedBox(height: Dimensions.PADDING_SIZE_SMALL),
                            InkWell(onTap: () {_showDaySelectionDialog(context, locationProvider);},
                                child: Container(
                                    height: 50,
                                    decoration: BoxDecoration(
                                      color: Colors.white,
                                    ...
                                    child: Padding(
                                        ...
                                        child: Row(children: [
                                          ...
                                          widget.waypoint!.day!,
                                              style: TextStyle(color: locationProvider.selectedDay == "Select the day" ? Colors.black45 : Colors.black87,
                                                  fontWeight: FontWeight.normal, fontSize: 16, fontFamily: 'Roboto')),
                                          const Spacer(),
                                          const Icon(Icons.view_day, color: Colors.black54, size: 25)])))),
                          ],
                        ),
                      ),
                    ),
              ...
              locationProvider.deleteWaypointLoading
                  ? Center(child: Padding(padding: const EdgeInsets.only(bottom: 20), child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(Theme.of(context).primaryColor),),),)
                  : Padding(
                      padding: const EdgeInsets.symmetric(
                          ...
                          showDialog(
                            ...
                            builder: (BuildContext context) {
                              return AlertDialog(
                                ...
                                content: Padding(padding: const EdgeInsets.symmetric(vertical: 15.0),
                                  child: Text('Are you sure you want to delete this waypoint?', style: TextStyle(fontWeight: FontWeight.w500, fontSize: 15,),),),
                                actions: [
                                  Padding(padding: const EdgeInsets.only(bottom: 10),
                                    child: Row(mainAxisAlignment: MainAxisAlignment.center,
                                      children: [
                                        MaterialButton(
                                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10), side: BorderSide(color: Colors.black26),),
                                          onPressed: () {
                                            FocusScope.of(context).unfocus();
                                            locationProvider.deleteWayPointInfo(widget.waypoint!.id!).then((value) {
                                              if (value.isSuccess) {Navigator.pop(context);
                                                showCustomSnackBar('Waypoint deleted successfully', context, isError: false,);
                                                locationProvider.getWaypointsList(context);
                                                Navigator.pop(context);
                                              } else {Navigator.pop(context);showCustomSnackBar('Something went wrong', context,);
                                              }
                                            });
                                          },
                                          child: Text("Yes"),
