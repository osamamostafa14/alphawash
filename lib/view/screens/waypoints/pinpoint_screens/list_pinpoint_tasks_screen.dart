...
class _PinPointsTasksScreenState extends State<PinPointsTasksScreen> {
  ...
  Widget build(BuildContext? context) {
    return Scaffold(
        appBar:
        Provider.of<ProfileProvider>(context!, listen: false).userInfoModel?.userType == 'admin'
            ? AppBar(
          title: Text('Pin Point Tasks', style: TextStyle(color: Theme.of(context).primaryColor)),
          centerTitle: true,
          elevation: 0.2,
        ...
        Consumer<WorkerProvider>(builder: (context, workerProvider, child) {
          ...
          if(Provider.of<ProfileProvider>(context).userInfoModel!.userType == 'admin'){
             hasPermissions = true;
          }
         else
           {
             if(Provider.of<ProfileProvider>(context).userInfoModel!.workerPermissions!=null){
               if(Provider.of<ProfileProvider>(context).userInfoModel!.workerPermissions!.showPinpoints == 1){
                 hasPermissions = true;
               }else{
                          ...
                          children: [
                            workerProvider.workerOldTasksListLoading
                                ...
                                : ListView.builder(
                                    ...
                                    itemCount: workerProvider.tasks.length,
                                    itemBuilder: (_, index) {
                                      final task = workerProvider.tasks[index];
                                      print(
